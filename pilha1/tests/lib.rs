...
pub fn Adicionar (&mut Self, objeto: T) -> Result<(), String> {
    match self.capacidade_maxima {
        Some(capacidade) if self.frutas.len() >= capacidade => {
            Err("Pilha Cheia".to_string())
...
pub fn ver_topo(&self) ->Option<&T> {
    self.objetos.last()
}
pub fn pilha_vazia(&self) -> bool {
    self.frutas.is_empty()
}
...
pub fn pilha_cheia(&self) -> bool {
    match.self.capacidade_maxima {
         some(capacidade) => self.frutas.len() >= capacidade,
         None => false
    }
}
